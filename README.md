...
A separate dated audit of training throughput, the data path, and the Optuna search is in `CLAUDE.md` under "Efficiency and tuning audit". Acted on so far: per-epoch Optuna pruning, an in-memory sample cache, standardized targets under a linear head, a Spearman metric for the rank-correlation gate, a single-objective `objective_2`, and the removal of dead search dimensions and per-batch `clear_memory()` calls. Studies must now supply their own pruner and use `direction="minimize"`. Still open: the cache key, the gradient metrics hardcoded into the evaluation loop, AMP, and a pairwise ranking loss. None of it is measured yet — the changes are syntax-checked only.
